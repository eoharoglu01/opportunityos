-- OpportunityOS initial Supabase schema
-- This migration creates the core catalog and personalization tables used by the app.
...
-- Stores that publish products and prices.
...
-- Core products listed on the platform.
...
-- Current price information per product and store.
...
-- Historical prices for trend analyses.
...
-- Useful indexes for search and joins.
...
create index if not exists idx_alerts_active on public.price_alerts(is_active);
...
-- Catalog data is publicly readable.
create policy if not exists "Public read access for categories" on public.categories
  for select using (true);

create policy if not exists "Public read access for stores" on public.stores
  for select using (true);

create policy if not exists "Public read access for products" on public.products
  for select using (true);

create policy if not exists "Public read access for product prices" on public.product_prices
  for select using (true);

create policy if not exists "Public read access for price history" on public.price_history
  for select using (true);

-- Users can manage their own profile.
create policy if not exists "Users can read and update their own profile" on public.profiles
