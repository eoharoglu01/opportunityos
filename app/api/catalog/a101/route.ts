  ...
  return NextResponse.json(
    {
      ...
      message: collectionResult.success
        ? `${collectionResult.collectedCount} A101 ürünü toplandı.`
        : "A101 ürünleri toplanamadı.",
...
export async function POST(request: Request) {
  try {
    const body =
      (await request.json()) as A101RequestBody;
    ...
    if (!collectionResult.success) {
      return NextResponse.json(
        {
          success: false,
          message: "A101 ürünleri toplanamadı.",
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "A101 katalog aktarımı sırasında bilinmeyen hata oluştu.",
      },
      {
        status: 500,
      },
